...
export default async function CharacterPage({ params }: Props) {
  ...
  return (
    <main className="overflow-x-clip px-6 py-16">
      {/* 中央見出し (9-nine のキャラ個別ページ準拠) */}
      <div className="text-center">
        <p className="text-grad-primary font-display text-5xl font-semibold tracking-[0.075em] sm:text-6xl">
          CHARACTER
        </p>
        <p className="mt-1 text-sm font-medium tracking-[0.3em] text-nine-blue">キャラクター</p>
      </div>

      {/* パンくず */}
      <nav
        aria-label="パンくず"
        className="mx-auto mt-12 max-w-5xl font-oswald text-xs tracking-[0.2em] text-nine-blue"
      >
      ...
      <section className="relative mx-auto mt-24 max-w-5xl">
        ...
        <div className="relative border border-nine-blue/30 bg-white/70">
          ...
          <div className="relative grid gap-6 lg:grid-cols-[minmax(0,400px)_1fr]">
            {/* 立ち絵は枠の上へはみ出させる */}
            <div className="flex items-end justify-center px-6 lg:-mt-16">
              {character.image ? (
                <Image
                  src={character.image}
                  alt={character.name}
                  ...
                  height={1400}
                  priority
                  className="h-[420px] w-auto object-contain object-bottom sm:h-[560px]"
                />
              ) : (
